...
public class Network : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < neurons.Length - 1; i++) {
            ...
            for (int j = i + i; j < neurons.Length; j++) {
                ...
                if (distance < 15) {
                    ...
                    if (spring.distance < 10) {
                        spring.distance = 10;
                    }
                    spring.connectedBody = neurons[j].GetComponent<Rigidbody2D>();
                    // Synapse s = new Synapse(0.1f, b);
                    GameObject s = new GameObject();
                    s = Instantiate(Resources.Load("Synapse")) as GameObject;
                    s.GetComponent<Synapse>().strength = 0.1f;
                    s.GetComponent<Synapse>().other = b;
                    // a.outputs.Add(new Synapse(0.1f, b));
